...
// 구글 로그인 성공 후에 호출되는 부분. JWT 발급까지 다 하고 프론트엔드로 보내주는 부분
// 의문점 : JSON 응답이 아니라 왜 redirect를 쓰는가?
// OAuth2의 flow가 브라우저 리다이렉트 기반이라서 JSON 응답을 보낼수 없다. URL 쿼리 파라미터에
// JWT를 담아 프론트엔드로 보내고, 프론트에서 파싱해야 한다.
@Slf4j
@Component
...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        User user = userRepository.findByEmail(email)
                .orElseThrow(() -> new RuntimeException("해당 사용자를 찾을수 없습니다: " + email));

        String token = jwtService.generateToken(user.getEmail(), user.getRole().name());

        // 이상의 프론트엔드로 리다이렉트 (토큰 + 사용자 정보를 쿼리 파라미터로 전달)
